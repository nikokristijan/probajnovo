...
/**
 * "Static Mesh Gradient" — made with the 21st.dev Shader Builder.
 ...
 *
 * Plain WebGL1 fullscreen-triangle renderer, no libraries. All the "feel"
 * sliders (speed/zoom/intensity/warp/contrast/brightness/saturation/hue/
 * vignette/grain) are baked into the packed uniforms below exactly as
 * produced by the shader builder — see the fragment source's own header
 * comment for what each packed vector means.
 */
...
// Colours (low → high), see header comment above for the OKLab-mixed palette.
const PALETTE_HEX = ["#02010A", "#04052E", "#3D2C8D", "#916BBF"];
...
export default function ShaderBackground({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    const gl = (canvas.getContext("webgl", { antialias: true, alpha: false }) ||
      canvas.getContext("experimental-webgl", { antialias: true, alpha: false })) as
    ...
    // Fullscreen triangle: one triangle covering the whole clip space is
    // cheaper than a quad and needs no index buffer.
    ...
    function render() {
      ...
      // Packed uniforms exactly as specified for the "Static Mesh Gradient"
      // preset (speed 87, zoom 30, intensity 41, warp 0, contrast 38,
      // brightness 76, saturation 59, hue 210°, vignette 0, grain 18, cursor off).
      gl!.uniform4f(uScene, canvas!.width, canvas!.height, t * -1.75, 4.0);
      gl!.uniform4f(uShape, 1.1, 0.41, 0.42, 0.0);
      gl!.uniform4f(uSurface, 1.82, 0.94, 0.26, 1.18);
      gl!.uniform4f(uFinish, 3.67, 0.0, 0.008, 0.06);
      gl!.uniform4f(uTransform, 7894.0, 0.0, 0.2, 1.0);
      gl!.uniform4f(uSpace, -0.15, -0.29, 0.0, 0.0);
      gl!.uniform4f(uCursor, 0.0, 0.0, 0.76, 0.43);
  ...
  return <canvas ref={canvasRef} aria-hidden="true" className={className ?? "block h-full w-full"} />;
}
